...
function RenderMotivation({ content }: { content: string }) {
    ...
    return (
        <div className="prose prose-sm max-w-none">
            {blocks.map((block: { type: string; id: string; data?: { level?: number; text?: string; style?: string; items?: string[] } }) => {
                switch (block.type) {
                    case 'header': {
                        const level = Math.min(6, Math.max(1, block.data?.level ?? 1));
                        const Tag = `h${level}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';
                        return <Tag key={block.id} className="font-bold my-2" style={{ fontSize: level === 1 ? '1.5em' : level === 2 ? '1.25em' : '1.1em' }}>{escapeHtml(block.data?.text ?? '')}</Tag>;
                    }
                    case 'paragraph':
                        return <p key={block.id} className="my-2">{escapeHtml(block.data?.text ?? '')}</p>;
                    case 'list': {
                        const listStyle = block.data?.style === 'ordered' ? 'ordered' : 'unordered';
                        const ListTag = listStyle === 'ordered' ? 'ol' : 'ul';
                        return (
                            <ListTag key={block.id} className="list-inside my-2" style={{ listStyleType: listStyle === 'ordered' ? 'decimal' : 'disc', paddingLeft: '20px' }}>
                    ...
                    case 'quote':
                        return <blockquote key={block.id} className="border-l-4 border-gray-300 pl-4 italic my-2">{escapeHtml(block.data?.text ?? '')}</blockquote>;
                    case 'delimiter':
