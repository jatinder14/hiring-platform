...
export default function CreateJobForm() {
    ...
    useEffect(() => {
        if (session?.user && typeof session.user === 'object' && 'name' in session.user && session.user.name) {
            setFormData(prev => ({ ...prev, company: String(session.user.name) }));
        }
    }, [session]);
    ...
    const isStep1Valid = formData.title?.trim() && formData.salaryMin && formData.description?.trim() && formData.expMin;
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', borderBottom: '1px solid #f3f4f6', paddingBottom: '32px' }}>
                                    ...
                                    <div style={{ textAlign: 'right' }}>
                                        ...
                                        <p style={{ fontSize: '20px', fontWeight: '800', color: '#111827' }}>
                                            {(() => {
                                                const sym = CURRENCIES.find(c => c.code === formData.currency)?.symbol ?? '';
